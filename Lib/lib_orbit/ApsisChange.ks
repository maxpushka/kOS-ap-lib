function ApChange {
    runpathonce("0:/kOS_ap_lib/Lib/lib_phys/VisVivaCalc.ks")
    ...
    local v1 is VisVivaCalc(targetAp,ship:altitude:semimajoraxis+(targetAp-apoapsis)).
    local dV is v1-v0.
	...
	clearscreen.
	sas on.
	if autowarp = true {kuniverse:timewarp:warpto(t_burn/2+15).}
    until ETA:periapsis < t_burn/2+1 {
        print "Burn in " + ETA:periapsis at(0,0).
